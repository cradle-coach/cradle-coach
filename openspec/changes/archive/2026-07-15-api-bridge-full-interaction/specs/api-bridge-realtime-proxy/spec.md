# Spec: API Bridge Realtime 代理

API Bridge 必须将 MiniCPM-o-Demo 全部 5 种交互模式代理到 OpenBMB Cloud API。
...
API Bridge MUST 双向转发所有 `input.append` 和 `session.close` 消息。

#### Scenario: 标准消息转发

- **WHEN** 客户端发送 `input.append {messages, streaming, generation}`
- **THEN** API Bridge 原样转发到 Cloud API

#### Scenario: 会话关闭

- **WHEN** 客户端发送 `session.close`
- **THEN** API Bridge 转发到 Cloud API 并关闭连接
