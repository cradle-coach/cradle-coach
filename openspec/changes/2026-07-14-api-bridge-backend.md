...
- 新增 `core/processors/api_bridge_backend.py`：`ApiBridgeBackend` 类，实现 `load_model()` 和 WebSocket 协议处理（`session.init` / `input.append` / `session.close`）
- 新增 `minicpmo45_service/api_bridge_server.py`：API Bridge Backend 的启动入口（独立进程，类似现有 backend server）
- 修改 `config.example.json`：新增 `api_bridge` 配置节（`api_endpoint`、`api_key`、`api_model`）
- 修改 `docker-compose.yml`：新增 `api-bridge` 服务定义
